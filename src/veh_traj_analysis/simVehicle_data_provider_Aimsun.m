classdef simVehicle_data_provider_Aimsun 
    properties
    ...
    methods ( Access = public )
        ...
        function [this]=simVehicle_data_provider_Aimsun(inputFolderLocation, outputFolderLocation,listSections, timePeriod, distance)
            %% This function is to obtain the sensor data
            
            % First, set default input and output folders
            this.inputFolderLocation=findCalibrationFolder.temp;
            this.outputFolderLocation=findCalibrationFolder.outputs;
        ...
        function [data_out]=get_statistics_for_section_time(this, listOfSections, timePeriod, distance)
            ...
            for i=1:numOfSections
                ...
                % Load data file
                dataFile=fullfile(this.inputFolderLocation,sprintf('SimVehInSection_%d.mat',sectionID));
                if(exist(dataFile,'file'))
                    ...
                    if(isempty(tmp_data)) % Data for that day not found
                        ...
                        % link
                        downSections=unique(turning_data(:,15));
                        numDownLinks=length(downSections);
                        centroidLaneByDownSection=[];
                        for j=1:numDownLinks
                            dataByDownSection=turning_data(turning_data(:,15)==downSections(j),:);
